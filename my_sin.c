...
int main(void) {
  ...
  /* manuel solution for the problem when the input is 25 or -25 */
  if (myabs(x) == 25) {
    if (x > 0) res_my_sin = my_sin(Y);
    else res_my_sin = my_sin(-Y);
  }
  /* if the input is legel: */
  else {
    res_my_sin = my_sin(x);
  }
  ...
  /* if x is not in legel values, then we will have a stack overflow error, and for that case we wont compute sin using my_sin */
  else {
    printf("the value you entered is not in the legel range, please try again");
  }
  return 0;
